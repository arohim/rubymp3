...
class BackendsControllerTest < ActionController::TestCase
  ...
  test "should create backend" do
    assert_difference('Backend.count') do
      post :create, backend: { album: @backend.album, artist: @backend.artist, cover: @backend.cover, mp3file: @backend.mp3file, title: @backend.title }
    end
  ...
  test "should update backend" do
    patch :update, id: @backend, backend: { album: @backend.album, artist: @backend.artist, cover: @backend.cover, mp3file: @backend.mp3file, title: @backend.title }
    assert_redirected_to backend_path(assigns(:backend))
  end

  test "should destroy backend" do
    assert_difference('Backend.count', -1) do
      delete :destroy, id: @backend
    end
